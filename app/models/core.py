...
class User(Base):
    ...
    total_diamonds_earned: Mapped[int] = mapped_column(Integer, default=0)  # برای محاسبه VIP - lifetime
    vip_level_id: Mapped[int | None] = mapped_column(ForeignKey("vip_levels.id"), nullable=True)
    ...
    vip_level: Mapped["VIPLevel"] = relationship(lazy="selectin")
...
class Setting(Base):
    """تنظیمات قابل‌تغییر از پنل ادمین بدون نیاز به دیپلوی مجدد"""
...
class VIPLevel(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    name: Mapped[str] = mapped_column(String(32))  # Bronze/Silver/Gold/Diamond
    min_total_earned: Mapped[int] = mapped_column(Integer)  # حداقل الماس کسب‌شده در طول عمر برای رسیدن به این سطح
